import { useEffect, useState } from "react";
import PageShell from "../components/layout/PageShell";
import { apiGet } from "../lib/api";
import { BadgeDollarSign, BarChart3, CheckCircle2, ShieldCheck } from "lucide-react";

const defaultEligibility = {
  financing_eligible_amount: 0,
  trade_score: 0,
  total_trade_value: 0,
  total_deals: 0,
};

export default function Financing() {
  const [requests, setRequests] = useState([]);
  const [companies, setCompanies] = useState([]);
  const [eligibility, setEligibility] = useState(defaultEligibility);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function load() {
      try {
        const userId = Number(localStorage.getItem("harvestlink_user_id"));
        const userRole = localStorage.getItem("harvestlink_role");

        // Fetch all companies so we can look up names in the table
        const allCompanies = await apiGet("/companies");
        setCompanies(allCompanies);

        // Resolve the logged-in user's company for the eligibility score
        let eligibilityCompanyId = null;
        if (userId) {
          const ownedCompanies = await apiGet(`/companies/owner/${userId}`);
          // Prefer exporter company; fall back to first owned company
          const myCompany =
            ownedCompanies.find((c) => c.type === "exporter") ||
            ownedCompanies.find((c) => c.type === "finance_partner") ||
            ownedCompanies[0] ||
            null;
          eligibilityCompanyId = myCompany?.id ?? null;
        }

        // Build both requests in parallel
        const [financeRequests, score] = await Promise.all([
          apiGet(
            userRole === "admin" || userRole === "finance_partner"
              ? "/financing"
              : eligibilityCompanyId
              ? `/financing?exporter_company_id=${eligibilityCompanyId}`
              : "/financing"
          ),
          eligibilityCompanyId
            ? apiGet(`/financing/eligibility/${eligibilityCompanyId}`)
            : Promise.resolve(defaultEligibility),
        ]);

        setRequests(financeRequests);
        setEligibility(score || defaultEligibility);
      } catch (error) {
        console.warn("Financing data unavailable:", error.message);
      } finally {
        setLoading(false);
      }
    }
    load();
  }, []);

  function companyName(companyId) {
    const company = companies.find((c) => c.id === companyId);
    return company?.name || `Company #${companyId}`;
  }

  return (
    <PageShell>
      <main className="mx-auto max-w-7xl px-4 py-10 lg:px-6">
        {/* Hero */}
        <section className="overflow-hidden rounded-[2rem] bg-harvest-green text-white shadow-soft">
          <div className="grid gap-8 p-8 lg:grid-cols-2 lg:p-12">
            <div>
              <div className="inline-flex rounded-full bg-white/10 px-4 py-2 text-sm font-bold">
                Trade Finance Layer
              </div>
              <h1 className="mt-5 text-5xl font-black">
                Access working capital based on real trade activity.
              </h1>
              <p className="mt-4 max-w-xl text-white/80">
                HarvestLink uses completed deals, payment performance, buyer relationships, and trade
                volume to determine financing eligibility.
              </p>
            </div>

            {/* Eligibility card */}
            <div className="rounded-[2rem] bg-white p-6 text-harvest-green">
              {loading ? (
                <div className="animate-pulse space-y-3">
                  <div className="h-4 w-32 rounded bg-gray-200" />
                  <div className="h-14 w-48 rounded bg-gray-200" />
                  <div className="h-10 w-full rounded bg-gray-200" />
                </div>
              ) : (
                <>
                  <div className="text-sm font-bold text-gray-500">Your estimated eligibility</div>
                  <div className="mt-2 text-6xl font-black">
                    ${eligibility.financing_eligible_amount?.toLocaleString?.() ?? "—"}
                  </div>
                  <div className="mt-4 rounded-2xl bg-harvest-soft p-4 font-bold">
                    Trade score: {eligibility.trade_score ?? "—"}
                  </div>
                </>
              )}
            </div>
          </div>
        </section>

        {/* Stats */}
        <div className="mt-8 grid gap-5 md:grid-cols-4">
          {[
            ["Trade value", `$${eligibility.total_trade_value?.toLocaleString?.() ?? 0}`, BadgeDollarSign],
            ["Total deals", eligibility.total_deals ?? 0, CheckCircle2],
            ["Trade score", eligibility.trade_score ?? 0, BarChart3],
            ["Risk layer", "Data-backed", ShieldCheck],
          ].map(([label, value, Icon]) => (
            <div key={label} className="rounded-3xl bg-white p-6 shadow-sm">
              <Icon className="text-harvest-orange" />
              <div className="mt-3 text-3xl font-black text-harvest-green">{value}</div>
              <div className="text-sm text-gray-500">{label}</div>
            </div>
          ))}
        </div>

        {/* Requests table */}
        <section className="mt-8 rounded-3xl bg-white p-6 shadow-sm">
          <h2 className="text-2xl font-black text-harvest-green">Financing Requests</h2>

          {loading ? (
            <div className="mt-5 animate-pulse space-y-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="h-10 rounded bg-gray-100" />
              ))}
            </div>
          ) : requests.length === 0 ? (
            <div className="mt-6 rounded-2xl bg-harvest-soft p-6 text-center text-gray-500">
              <p className="font-bold">No financing requests yet.</p>
              <p className="mt-1 text-sm">
                Financing requests submitted from a deal room will appear here.
              </p>
            </div>
          ) : (
            <div className="mt-5 overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="text-gray-500">
                  <tr>
                    <th className="p-3">Exporter</th>
                    <th className="p-3">Requested</th>
                    <th className="p-3">Eligible</th>
                    <th className="p-3">Score</th>
                    <th className="p-3">Status</th>
                  </tr>
                </thead>
                <tbody>
                  {requests.map((r) => (
                    <tr key={r.id} className="border-t">
                      <td className="p-3 font-bold text-harvest-green">
                        {r.exporter_name || companyName(r.exporter_company_id)}
                      </td>
                      <td className="p-3">${r.requested_amount?.toLocaleString?.()}</td>
                      <td className="p-3">${r.eligible_amount?.toLocaleString?.()}</td>
                      <td className="p-3">{r.score}</td>
                      <td className="p-3">
                        <span className="rounded-full bg-green-100 px-3 py-1 text-xs font-bold text-green-700">
                          {r.status}
                        </span>
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
          )}
        </section>
      </main>
    </PageShell>
  );
}
