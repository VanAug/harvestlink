...
import { LockKeyhole, MapPin, MessageSquare, PackageCheck, Star, Trash2, Truck } from "lucide-react";
...
export default function Deals() {
  ...
  const [docForm, setDocForm] = useState({ document_type: "Invoice", title: "", notes: "" });
  ...
  const activeDeal = useMemo(
    () => deals.find((deal) => deal.id === activeId) || deals[0],
    [deals, activeId]
  );
  ...
  async function load() {
    ...
    if (userId) {
      ...
      } else if (role === "exporter" || role === "supplier") {
      ...
      if (selectedCompany) {
        ...
        path =
          selectedCompany.type === "buyer"
            ? `/deals?buyer_company_id=${selectedCompany.id}`
            : `/deals?exporter_company_id=${selectedCompany.id}`;
      }
    }
  ...
  async function submitDocument(e) {
    ...
    setDocForm({ document_type: "Invoice", title: "", notes: "" });
    setSelectedFileName("");
    ...
    <PageShell>
      <main className="mx-auto max-w-7xl px-4 py-10 lg:px-6">
        <div className="mb-8 rounded-[2rem] bg-white p-8 shadow-soft">
          ...
          {notice && (
            <div className="mt-5 rounded-2xl bg-harvest-soft p-3 text-sm font-bold text-harvest-green">
              {notice}
            </div>
          )}
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
            ...
            {deals.length ? (
              deals.map((deal) => (
                <button
                  ...
                  className={`w-full rounded-3xl bg-white p-5 text-left shadow-sm hover:shadow-soft ${activeDeal?.id === deal.id ? "ring-2 ring-harvest-orange" : ""}`}
                >
                  ...
                  <p className="mt-2 text-sm text-gray-600">
                    {deal.quantity} {deal.unit} · {deal.destination_country}
                  </p>
                  <p className="mt-2 font-black text-harvest-orange">
              ...
              <div className="rounded-3xl bg-white p-6 text-center text-gray-600 shadow-sm">
                ...
                <p className="mt-2 text-sm">
                  Create or join a deal to start tracking messages, documents, and escrow.
                </p>
              </div>
            )}
          </section>
          ...
          <section className="rounded-3xl bg-white p-6 shadow-sm lg:col-span-2">
            <div className="flex flex-col justify-between gap-4 border-b pb-5 md:flex-row md:items-start">
              <div>
                <h2 className="text-3xl font-black text-harvest-green">
                  {activeDeal ? activeDeal.product_name : "No deal selected"}
                </h2>
                <p className="mt-2 text-gray-600">
                  {activeDeal
                    ? `${activeDeal.quantity} ${activeDeal.unit} to ${activeDeal.destination_country}`
                    : "Select a deal from the left to view messages, documents, and status updates."}
                </p>
              </div>
              <span className="rounded-full bg-harvest-green px-4 py-2 text-sm font-bold text-white">
                Escrow: {activeDeal?.escrow_status || "N/A"}
              </span>
            </div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {/* Exporter-only actions */}
              {isExporter && (
                <>
                  <button
                    disabled={!activeDeal}
                    onClick={() => activeDeal && updateStatus("in_fulfillment")}
                    className="rounded-2xl bg-harvest-green px-4 py-2 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    Mark In Fulfillment
                  </button>
                  <button
                    disabled={!activeDeal}
                    onClick={() => activeDeal && updateStatus("shipped")}
                    className="rounded-2xl bg-harvest-orange px-4 py-2 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    Mark Shipped
                  </button>
                  <button
                    ...
                    className="rounded-2xl border border-harvest-green px-4 py-2 text-sm font-black text-harvest-green disabled:cursor-not-allowed disabled:opacity-50"
                  >
              ...
              {isBuyer && (
                <Link
                  to={activeDeal ? `/deals/${activeDeal.id}/review` : "#"}
                  className="inline-flex items-center gap-2 rounded-2xl bg-yellow-500 px-4 py-2 text-sm font-black text-white hover:bg-yellow-600"
                >
                  <Star size={16} />
                  Rate &amp; Review
                </Link>
              ...
              {/* Both roles */}
              <Link
                to={activeDeal ? `/deals/${activeDeal.id}/tracking` : "#"}
                className="inline-flex items-center gap-2 rounded-2xl bg-blue-500 px-4 py-2 text-sm font-black text-white hover:bg-blue-600"
              >
                <MapPin size={16} />
                Track Shipment
              </Link>
            ...
            <div className="mt-6 grid gap-6 lg:grid-cols-2">
              ...
              <div className="rounded-3xl bg-harvest-soft p-5">
                ...
                <div className="mt-4 max-h-72 space-y-3 overflow-auto">
                  {messages.map((item) => (
                    <div key={item.id} className="rounded-2xl bg-white p-4 text-sm text-gray-700">
                ...
                <form onSubmit={sendMessage} className="mt-4 flex gap-2">
                  <input
                    ...
                    className="min-w-0 flex-1 rounded-2xl border border-gray-200 px-4 py-3"
                    placeholder={isBuyer ? "Message supplier..." : "Message buyer..."}
                  />
                  <button className="rounded-2xl bg-harvest-green px-4 py-3 font-bold text-white">
                    Send
                  </button>
                </form>
              </div>
              ...
              <div className="rounded-3xl bg-harvest-soft p-5">
                    ...
                    <div key={doc.id} className="rounded-2xl bg-white p-3 text-sm">
                      <div className="flex items-center justify-between gap-2">
                        <div>
                          ...
                          {/* Status badge replaces the misleading Local/Blob badge */}
                      ...
                      {doc.file_url && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="mt-2 inline-block text-sm font-bold text-harvest-green hover:underline"
                        >
                          View document ↗
                        </a>
                      )}
                      {doc.notes && <div className="mt-1 text-xs text-gray-500">{doc.notes}</div>}
                    </div>
                  ))}
                </div>
                ...
                <form onSubmit={submitDocument} className="mt-4 space-y-3">
                  ...
                  >
                    {["Invoice", "Packing List", "Certificate", "Bill of Lading", "Shipping Update"].map(
                  ...
                  <Input
                    ...
                    placeholder="Commercial invoice #1024"
                  />
                  <label className="block">
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                      <button
                        ...
                        className="inline-flex items-center justify-center rounded-2xl bg-harvest-green px-5 py-3 text-sm font-black text-white shadow-sm hover:bg-emerald-600"
                      >
                        Browse file
                      </button>
                  ...
                  <button className="w-full rounded-2xl bg-harvest-orange px-4 py-3 font-bold text-white">
                    Add Document
                  </button>
                </form>
              </div>
              ...
              <form onSubmit={requestFinancing} className="mt-6 rounded-3xl bg-white p-5 ring-1 ring-gray-100">
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-[180px_1fr_auto] md:items-end">
                  <Input
                    ...
                    type="number"
                    min="0"
                    value={financing.requested_amount}
                    onChange={(e) => setFinancing((c) => ({ ...c, requested_amount: e.target.value }))}
                    placeholder="10000"
                  />
                  <Input
                    required
                    label="Purpose"
                    value={financing.purpose}
                    onChange={(e) => setFinancing((c) => ({ ...c, purpose: e.target.value }))}
                    placeholder="Packaging and farmer aggregation for this order"
                  />
                  <button className="rounded-2xl bg-harvest-green px-5 py-3 font-black text-white">
                    Submit
                  </button>
                </div>
              </form>
