...
import { apiPost, login, register } from "../lib/api";

const roleOptions = [
  { value: "buyer", label: "Buyer", nextPath: "/buyer/profile" },
  { value: "exporter", label: "Exporter", nextPath: "/exporter/profile" },
  { value: "finance_partner", label: "Finance Partner", nextPath: "/financing" },
];
...
export default function Auth({ mode = "login" }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const data = isLogin ? await login(email, password) : await registerAndCreateCompany();
      navigate(nextPathForRole(data.role, isLogin));
    } catch (error) {
      ...
      <main className="mx-auto max-w-2xl px-4 py-16">
        <form onSubmit={handleSubmit} className="rounded-[2rem] bg-white p-8 shadow-soft">
          <div className="inline-flex rounded-full bg-harvest-soft px-4 py-2 text-sm font-bold text-harvest-green">
            {isLogin ? "Demo password: password123" : "Create your account and company profile"}
          </div>
          ...
          {/* Login — single column vertical layout */}
          {isLogin ? (
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2">
              <div className="md:col-span-2">
                <Input required label="Full Name" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Enter your name" />
              </div>
              <Input required label="Email Address" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@company.com" />
              <Input required label="Password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Create a password" />
              <Input required label="Confirm Password" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Repeat password" />
              ...
              <Input required label="Company Name" value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder="Company or business name" />
              <Input required label="Country" value={country} onChange={(e) => setCountry(e.target.value)} placeholder="Kenya" />
            </div>
          )}

          {message && <div className="mt-5 rounded-2xl bg-red-50 p-3 text-sm font-bold text-red-700">{message}</div>}

          <button disabled={submitting} className="mt-6 w-full rounded-2xl bg-harvest-green px-5 py-3 font-black text-white disabled:cursor-not-allowed disabled:bg-gray-400">
            {submitting ? "Please wait..." : isLogin ? "Login" : "Create Account"}
          </button>

          <div className="mt-5 text-center text-sm text-gray-600">
            {isLogin ? (
              <>New to HarvestLink? <Link className="font-bold text-harvest-green" to="/register">Create an account</Link></>
            ) : (
              <>Already have an account? <Link className="font-bold text-harvest-green" to="/login">Login</Link></>
            )}
          </div>
        </form>
      </main>
...
function nextPathForRole(role, isLogin) {
  if (role === "buyer") return isLogin ? "/buyer-dashboard" : "/buyer/profile";
  if (role === "admin") return "/admin-dashboard";
  if (role === "finance_partner") return "/financing";
  return isLogin ? "/exporter-dashboard" : "/exporter/profile";
}
